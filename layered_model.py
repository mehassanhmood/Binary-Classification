...
def define_dense_model_single_layer(input_length, activation_f='sigmoid', output_length=1):
    ...
    model.add(layers.Dense(units=output_length, input_shape=(input_length,),activation=activation_f))
    return model
...
def define_dense_model_with_hidden_layer(input_length, 
    ...
    model = keras.Sequential()
    model.add(layers.Dense(units = hidden_layer_size,input_shape=(input_length, ),activation=activation_func_array[0]))
